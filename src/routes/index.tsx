...
import { Sparkles, Wand2, AlertCircle, Cpu, Play } from "lucide-react";
...
import type { CharacterInput, NarrationAudio, VideoExportOptions, VideoExportProgress, ExportedVideo, SceneWithNarration } from "@/lib/types/character.types";
...
function HomePage() {
  ...
  const [character, setCharacter] = useState<CharacterInput | null>(null);
  ...
  const handleCharacterAdd = (newCharacter: CharacterInput) => {
    ...
    try {
      ...
      const res = await generateNarrationFn({
        data: {
          ...
          options: {
            service: "openai", // Default to OpenAI, can be made configurable
          },
        },
      });

      if (res.error || !res.result) {
        console.error("Narration generation failed:", res.error);
      } else {
        const newNarrations = [...sceneNarrations];
        newNarrations[sceneIndex] = res.result;
        setSceneNarrations(newNarrations);
      }
    } catch (err) {
  ...
  const handleExportVideo = async (
    ...
    // Convert scenes to SceneWithNarration
      ...
      <div className="mx-auto max-w-4xl px-4 sm:px-6 py-12 sm:py-20">
        ...
        <GlassCard variant="neon" className="p-6 sm:p-8 animate-float-up" style={{ animationDelay: "100ms" }}>
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="pt-2">
              <CharacterInputForm onCharacterAdd={handleCharacterAdd} disabled={loading} />
            </div>
